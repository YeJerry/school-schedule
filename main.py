...
st.title("📱 學校自動排課系統 (支援職稱鎖定與多節次連寫)")
...
st.sidebar.write("💼 核心升級：**排課限制節次支援填寫「第1-3節」或「第2,3節」**")
...
with main_tabs[0]:
    ...
    st.markdown("💡 *請確保您的 Excel 包含：`任教班級`、`老師姓名`、`科目`、`每週堂數`、`需要連排(對/錯)`、`課程屬性`、`職稱` 欄位。*")
    uploaded_file = st.file_uploader("O 選擇【開課明細（含職稱）】Excel 檔案", type=["xlsx"], key="main_file")
...
with main_tabs[1]:
    ...
    st.markdown("💡 *對象名稱可填寫「主任」、「組長」等職稱，或「七年級」；節次可直接連寫如「第1-3節」或「第2,3,4節」。*")
    
    example_df = pd.DataFrame([
        {"對象分類": "個人/班級", "對象名稱": "主任", "星期": "週二", "節次": "第1-3節", "限制原因": "主管會報集體鎖定"},
        {"對象分類": "個人/班級", "對象名稱": "七年級", "星期": "週五", "節次": "第5,6節", "限制原因": "跨班聯課活動"},
        {"對象分類": "個人/班級", "對象名稱": "陳組長", "星期": "週四", "節次": "第2-4節", "限制原因": "公出開會"},
        {"對象分類": "領域研習", "對象名稱": "數學", "星期": "週三", "節次": "第5-6節", "限制原因": "領域會議"}
    ])
    towrite = io.BytesIO()
    ...
    st.download_button(
        label="📥 下載「支援職稱與多節連寫之 Excel 範例範本」",
        data=towrite.getvalue(),
        file_name="高級排課限制填寫範本.xlsx",
    ...
    if uploaded_rules_file is not None:
        try:
            ...
            st.success(f"🎉 限制條件匯入完成！成功將多節次與職稱規則展開解析為 {success_count} 個獨立禁排時段！")
        ...
        if st.button("🔥 啟動高級階梯權重平衡排課引擎", type="primary", use_container_width=True):
            try:
                ...
                if success_fully_placed:
                    st.success(f"🏆 【策略行政排課成功】系統已完美避開職稱（如主管會報）禁排時段，並順利解鎖完美課表！")
                else:
                    st.error(f"❌ 迭代 {max_attempts} 次失敗，仍有 {len(final_unplaced)} 節課無法排入。主管會報卡住太多課，建議放寬限制。")
                    st.session_state.total_input_lessons = total_input_lessons
