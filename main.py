...
with main_tabs[1]:
    ...
    example_df = pd.DataFrame([
        ...
        {"對象分類": "領域研習", "對象名稱": "體育", "星期": "週四", "節次": "第3-4節", "限制原因": "體育領域時間(側邊欄若勾選可排課則不卡)"}
    ])
    towrite = io.BytesIO()
    ...
    if uploaded_rules_file is not None:
        try:
            ...
            for _, row in rules_df.iterrows():
                ...
                name = str(row.get("對对象名稱" if "對对象名稱" in row else "對象名稱", "")).strip()
            ...
            try:
                ...
                while attempt < max_attempts:
                    ...
                    def can_place(c, t, s, d, p, attr, is_part_of_double=False, strict_level=1, ignore_domain=False):
                        ...
                        if strict_level < 4:
                            # 💥 核心判斷優化：如果該領域被加入「寬限允許排課清單」，則直接跳過禁排限制！
                            if not ignore_domain:
                                dom = subj_to_domain.get(s, s)
                                if dom not in allow_排課_domains: # 只有「不在寬限清單內」的領域才要實施禁排
